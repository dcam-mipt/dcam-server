    ...
    if (user) {
        ...
        if (is_free) {
            ...
            await Mailer.sendEmail({
                ...
                html: `
                    <html>
                    <div style={{ marginTop: '0.1vw', }} >Вы запланировали стирку 🧺:</div>
                    <div style={{ marginTop: '0.1vw', }} >Дата: ${days_of_week_short[moment(+request.params.timestamp).tz(`Europe/Moscow`).isoWeekday() - 1]} ${moment(+request.params.timestamp).tz(`Europe/Moscow`).format(`DD.MM.YY`)}</div>
                    <div style={{ marginTop: '0.1vw', }} >Время: ${moment(+request.params.timestamp).tz(`Europe/Moscow`).format(`HH:mm`)}</div>
                    <div style={{ marginTop: '0.1vw', }} >Общежитие: 7</div>
                    <div style={{ marginTop: '0.1vw', }} >Машинка: ${machine_index}</div>
                    <div style={{ marginTop: '0.2vw', }} >Спасибо, что пользуетесь услугами Студсовета ФПМИ!</div>
                    </html>
                `
            })
